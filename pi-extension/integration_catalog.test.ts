...
test("discovers reflection integrations and model catalogs without serial catalog probing", async () => {
	...
	const fetchJSON: JSONFetcher = async (url) => {
		...
		if (url.endsWith("/models.json")) {
			...
			maxActiveCatalogFetches = Math.max(
				maxActiveCatalogFetches,
				activeCatalogFetches,
			);
	...
	const discovered = await discoverIntegrationCatalogs(
		reflectionURL,
		fetchJSON,
	);
	...
	assert.equal(
		fetched.some((url) => url.includes(".exe.cloud")),
		false,
	);
	assert.ok(
		maxActiveCatalogFetches > 1,
		`catalog fetches were serial; max active was ${maxActiveCatalogFetches}`,
	);
});

test("retries transient reflection and model-catalog failures", async () => {
	...
	const fetchJSON: JSONFetcher = async (url) => {
		...
		if (url.endsWith("/integrations")) {
			return { integrations: [{ type: "llm", name: "llm" }] };
		}
		return { schema_version: 1, models: [] };
	};
	...
	assert.equal(discovered.found, true);
	assert.equal(discovered.integrations[0]?.name, "llm");
	assert.equal(attempts.get(reflectionURL), 2);
	assert.equal(attempts.get("https://llm.int.exe.xyz/models.json"), 2);
});
...
test("keeps integration ownership when reflection succeeds but catalogs fail", async () => {
	...
	const fetchJSON: JSONFetcher = async (url) => {
		if (url.endsWith("/integrations"))
			return { integrations: [{ type: "llm", name: "broken" }] };
	...
	assert.equal(
		discovered.integrations[0]?.baseURL,
		"https://broken.int.exe.xyz",
	);
...
test("discovers team llm integrations through team hosts", async () => {
	...
	const fetchJSON: JSONFetcher = async (url) => {
		...
		if (
			url === "https://shared.team.exe.xyz/models.json" ||
			url === "https://shared.int.exe.xyz/models.json"
		) {
			return { schema_version: 1, models: [] };
		}
		return undefined;
...
test("enables full Pi thinking levels for gateway GPT-5.6 variants", () => {
		...
		[
			{
				...
				catalog: {
					schema_version: 1,
					models: [customModel("openai", "gpt-5.6-luna", ["openai_responses"])],
				},
			},
		],
		undefined,
...
test("routes arbitrary providers by client protocol priority", () => {
		...
		[
			{
				...
				catalog: {
					schema_version: 1,
					models: [
						customModel("openrouter", "anthropic/claude-sonnet-4.6", [
							"openai_chat",
						]),
						...
						customModel("anthropic-proxy", "claude-custom", [
							"openai_chat",
							"anthropic_messages",
						]),
						customModel("embedding-only", "embedding-model", [
							"openai_embeddings",
						]),
					],
				},
	...
	assert.equal(
		multi.config.models?.[0]?.baseUrl,
		"https://llm.int.exe.xyz/multi-protocol/v1",
	);
	...
	assert.equal(
		anthropicProxy.config.models?.[0]?.baseUrl,
		"https://llm.int.exe.xyz/anthropic-proxy",
	);
});
...
test("preserves duplicate model names and marks ChatGPT rewrites by generated model id", () => {
	...
	assert.deepEqual(
		...
		[
			"https://chatgpt-sub.int.exe.xyz/openai/v1",
			"https://managed-sub.int.exe.xyz/openai/v1",
		],
	);
});
...
test("formats prompt integration availability with at most two names", () => {
	assert.equal(
		integrationPromptAvailabilityLabel(["beta", "alpha"]),
		"alpha, beta",
	);
	assert.equal(
		integrationPromptAvailabilityLabel(["beta", "alpha", "gamma"]),
		"alpha, beta, ...",
	);
	assert.equal(
